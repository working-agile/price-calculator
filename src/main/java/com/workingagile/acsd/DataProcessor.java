...
public class DataProcessor {
	...
	protected void calculateDataInstance(boolean next) {
		...
		try {
			...
			List<Item> newList = processItems(next, items);

			updateItemsInDatabase(newList);

			list = new ArrayList<Item>(newList);

		} catch(Exception exception) {}
	}

	protected List<Item> processItems(boolean next, List<Item> items) {
		List <Item> newList = new ArrayList<Item>();
		for (Item item: items) {
			processItem(next, item, newList);
		}
		return newList;
	}

	protected void processItem(boolean next, Item item, List<Item> newList) {
	...
	private static List<Item> getItemsFromDatabase() throws SQLException {
	...
	private void updateItemsInDatabase(List<Item> newList) throws SQLException {
